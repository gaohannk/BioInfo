...
public class Problem2 {
	 ...
	 */
	public static int NumberOfBreakpoints(List<String> input) {
		int count = 0;
		int n = input.size();
		input.add(0, "0");
		input.add("+" + (n + 1));
		for (int i = 0; i < input.size() - 1; i++) {
			if (Integer.parseInt(input.get(i)) + 1 != Integer.parseInt(input.get(i + 1))) {
				count++;
			}
		}
		return count;
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(NumberOfBreakpoints(Arrays.stream(input).collect(Collectors.toList())));
	}
}
