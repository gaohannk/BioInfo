...
public class Problem5_2 {
	 ...
	 */
	// Pass Grader But logical not correct, the algorithm just append new edge at the end, not good for the following problem 6
	public static List<String> TwoBreakOnGenomeGraph(List<String> coloredEdges, int i1, int i2, int i3, int i4) {
		...
		for (String edge : coloredEdges) {
			int node = Integer.parseInt(edge.split(Delimiter)[0]);
			int node2 = Integer.parseInt(edge.split(Delimiter)[1]);
			if (node == i1 && node2 == i2 || node == i2 && node2 == i1 || node == i3 && node2 == i4 || node == i4 && node2 == i3) {
				continue;
			}
			afterBreak.add(edge);
		}
		afterBreak.add(i1 + Delimiter + i3);
		afterBreak.add(i2 + Delimiter + i4);
		return afterBreak;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		String[] indices = splits[1].split(Delimiter);
		...
		System.out.println(afterBreak.stream().map(e -> "(" + e + ")").collect(Collectors.joining(Delimiter)));
	}
}
